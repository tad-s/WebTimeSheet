            ...
            if ($_POST['action'] == 'register') {
                ...
                if (empty($err)) {
                    ...
                    if ($edit_id) {
                        ...
                        $stmt = $pdo->prepare($sql);
                        $stmt->bindValue(':user_no', $target_user['user_no'], PDO::PARAM_STR);
                        $stmt->bindValue(':user_name', $target_user['name'], PDO::PARAM_STR);
                        $stmt->bindValue(':t_flag', $modal_expense_type, PDO::PARAM_INT);
                        $stmt->bindValue(':date', $modal_date, PDO::PARAM_STR);
                        $stmt->bindValue(':from_name', $modal_from_name, PDO::PARAM_STR);
                        $stmt->bindValue(':to_name', $modal_to_name, PDO::PARAM_STR);
...
<body class="text-center bg-light">
    ...
    <form class="border rounded bg-white form-time-table" method="get">

        <h1 class="h3 my-3">交通費確認・編集</h1>
        
        <!-- ユーザー情報表示 -->
        <div class="user-info text-left">
            <small class="text-muted">
                対象ユーザー: <?php echo h($target_user['user_no']); ?> - <?php echo h($target_user['name']); ?>
            </small>
        </div>
        ...
        <div class="float-right">
            <a class="btn btn-success mr-2" href="fare_export_csv.php?id=<?php echo $user_id ?>&m=<?php echo urlencode($yyyymm) ?>">
                <i class="fas fa-file-csv"></i> CSV出力
            </a>
            <a class="btn btn-primary mr-2" href="user_result.php?id=<?php echo $user_id ?>&m=<?php echo urlencode($yyyymm) ?>">
                <i class="fas fa-calendar-alt"></i> 勤務表に戻る
            </a>
            <a class="btn btn-secondary" href="user_list.php">
                <i class="fas fa-list"></i> 社員一覧
            </a>
        </div>
                    ...
                    <div class="modal-body">
                        <div class="container">
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" id="modal_date" name="modal_date" value="<?php echo $modal_date; ?>" required>
                            </div>
